...
async function queryOllama(prompt: string, opts?: { json?: boolean }): Promise<string> {
...
export async function gemini(prompt: string, opts?: { json?: boolean; language?: string }): Promise<string> {
  ...
  if (provider === "local") {
    return await queryOllama(prompt, opts);
  } else {
    try {
      ...
      console.warn(`     ⚠️ Gemini API failed: ${error.message || error}. Falling back to local Ollama...`);
      return await queryOllama(prompt, opts);
    }
  }
}
